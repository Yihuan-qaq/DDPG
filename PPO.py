    ...
    def update(self):
        ...
        while not COORD.should_stop():
            if GLOBAL_EP < EP_MAX:
                ...
                data = np.vstack(data)
                s, a, r = data[:, :S_DIM], data[:, S_DIM: S_DIM + A_DIM].ravel(), data[:, -1:]
...
class Worker(object):
    def __init__(self, wid):
        self.wid = wid
        self.env = Env_PPO(PHN, SOURCE_PATH_WAV, SOURCE_PATH_PHN)
        self.ppo = GLOBAL_PPO

    def work(self):
        global GLOBAL_EP, GLOBAL_RUNNING_R, GLOBAL_UPDATE_COUNTER, GLOBAL_R_MAX
        while not COORD.should_stop():
            s = self.env.reset()
            ep_r = 0
            avg_S = 0
            ...
            for t in range(EP_LEN):
                ...
                s_, r, done, asr_time = self.env.step(s, a, self.wid)
                ASR_TIME += asr_time
                # if done: r = -10
                buffer_s.append(s)
                buffer_a.append(a)
                ...
                if r > GLOBAL_R_MAX:
                    r_max_record = s
                    GLOBAL_R_MAX = r
                    epoch_record = GLOBAL_EP
                    print('\n temp_record_done_r', GLOBAL_R_MAX)
            ...
            GLOBAL_EP += 1
            print('AVG State:{}'.format(avg_S / EP_LEN))
            print('{0:.1f}%'.format(GLOBAL_EP / EP_MAX * 100), '|W%i' % self.wid, '|Ep_r: %.2f' % ep_r,
                  'ASR Time:'.format(ASR_TIME), 'EP Total Time:'.format(time.time() - t1))
...
if __name__ == '__main__':
    ...
    GLOBAL_UPDATE_COUNTER, GLOBAL_EP, GLOBAL_R_MAX = 0, 0, 0
    GLOBAL_RUNNING_R = []
